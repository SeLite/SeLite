...
window.addEventListener( "load", function(e) {
    ...
    if( document.location.search ) {
        ...
        if( match ) {
            ...
            modules[ moduleName ]= SeLiteSettings.loadFromJavascript( moduleName, undefined, true/** Don't cache, so that user's changes has an effect without restarting Firefox. */ );
    ...
    if( isEmptyObject(modules) ) {
        ...
        for( var i=0; i<moduleNames.length; i++ ) {
            var module= SeLiteSettings.loadFromJavascript( moduleNames[i], undefined, true/** Don't cache, so that user's changes has an effect without restarting Firefox. */ );
