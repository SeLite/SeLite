...
function change_or_comment_out() {
    ...
    then
        ...
        then
            ...
            sed -i -r "s/(\/\/)?(,\s*$field\s*:)\s*['\"]?[0-9.]*['\"]?/\2 \"$value\"/" extensions/$extension/chrome/content/SeLiteExtensionSequencerManifest.js
        else
            # comment out the line
            sed -i -r "s/(\/\/)?(,\s*$field\s*:.*)/\/\/\2/" extensions/$extension/chrome/content/SeLiteExtensionSequencerManifest.js
        fi
    else
        if [ "$value" ]
        then
